...
async function runEngine(
  ...
  parse: (html: string) => { total: number; counted: number; officialCount: boolean },
): Promise<EngineHit> {
  try {
    const html = await fetchText(url, { timeoutMs: 8000, headers });
    if (isAntiBot(html)) throw new Error('命中反爬验证');
    const { total, counted, officialCount } = parse(html);
    return { engine, ok: true, total, counted, officialCount };
  } catch (error) {
    return {
      engine,
      ok: false,
...
/** Bing 网页：span.sb_count「约 29,100 个结果」+ li.b_algo 结果条数 */
export function searchBingWeb(query: string): Promise<EngineHit> {
  ...
  return runEngine('Bing网页', url, {}, (html) => {
    ...
    const counted = $('li.b_algo').length;
    return { total: num > 0 ? num : counted, counted, officialCount: num > 0 };
  });
}
...
export function searchBaiduWeb(query: string): Promise<EngineHit> {
  ...
  return runEngine('百度网页', url, { Referer: 'https://www.baidu.com/' }, (html) => {
    const $ = cheerio.load(html);
    const counted = $('.c-container').length > 0 ? $('.c-container').length : $('.result').length;
...
/** 百度新闻：span.nums「百度为您找到相关资讯100个」+ .result 结果条数 */
export function searchBaiduNews(query: string): Promise<EngineHit> {
  ...
  return runEngine('百度新闻', url, {}, (html) => {
    ...
    const num = m ? toNum(m[1]) : 0;
    const counted = $('div.result, div.result-op').length > 0
...
/** 搜狗网页：p.num-tips「搜狗已为您找到约37,020条相关结果」+ .vrwrap 结果条数 */
export function searchSogouWeb(query: string): Promise<EngineHit> {
  ...
  return runEngine('搜狗网页', url, { Referer: 'https://www.sogou.com/' }, (html) => {
    ...
    const num = m ? toNum(m[1]) : 0;
    const counted = $('.vrwrap').length > 0 ? $('.vrwrap').length : $('.rb').length;
    return { total: num > 0 ? num : counted, counted, officialCount: num > 0 };
  });
}
