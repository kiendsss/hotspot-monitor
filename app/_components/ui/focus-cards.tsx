    ...
    <div className={cn("grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3", className)}>
      {items.map((child, i) => (
        <div
          ...
          className={cn(
            "rounded-2xl border border-[var(--line)] bg-[var(--paper)] p-5 shadow-[3px_3px_0_rgba(42,37,29,0.22)] transition-all duration-300",
            hovered !== null && hovered !== i
              ? "blur-[0.8px] opacity-70 scale-[0.98]"
              : "blur-0 opacity-100 scale-100",
            hovered === i ? "shadow-[4px_4px_0_var(--red-dark)] -translate-y-0.5" : ""
          )}
        >
          {child}
        </div>
